...
enum BillingClient {
    static func fetch(token: String) async throws -> UsageSnapshot {
        ...
        else if let p = config["credit_usage_percent"] as? Double { used = p }
        if let used, used <= 1.0001 { used = used * 100 }
        ...
        for p in products {
            ...
            var val = (p["usagePercent"] as? Double) ?? (p["usage_percent"] as? Double) ?? 0
